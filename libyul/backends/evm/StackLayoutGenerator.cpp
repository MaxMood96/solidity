...
namespace
{
/// @returns all stack too deep error that would occur when shuffling @a _source to @a _target.
...
vector<StackLayoutGenerator::StackTooDeep> StackLayoutGenerator::reportStackTooDeep(CFG::BasicBlock const& _entry) const
{
	...
	breadthFirstSearch.run([&](CFG::BasicBlock const* _block, auto _addChild) {
		...
		// Do not attempt to create the exit layout here, since the code generator will directly move to the
		// target entry layout.
